...
for(const arr of StudentDetails){
const tr = document.createElement("tr");

tr.innerHTML =` <td>${arr.id}</td>
               <td>${arr.first_name} ${arr.last_name}</td>
                <td>${arr.gender}</td>
                <td>${arr.class}</td>
                <td>${arr.marks}</td>
                <td>${arr.passing ? 'passing': 'failed'}</td>
                <td>${arr.email}</td>
                `;
...
StudentDetails.sort((a,b) =>a.marks- b.marks);
   displayStudent(StudentDetails);
}

function SortByClass(){
    
    StudentDetails.sort((c,d) =>c.class- d.class);
    displayStudent(StudentDetails);
    }


function SortByPassing(){
    const passingStudent=StudentDetails.filter(person => person.passing)
     displayStudent(passingStudent);
}
